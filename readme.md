...
## Configure the Base Path (Optional)
By default, the server uses the `notebook/` directory relative to the script location. 
To use a custom path, set the `MCP_BASE_PATH` environment variable:

```bash
export MCP_BASE_PATH="/path/to/your/notebook/directory"
```

Or pass it when running the MCP Inspector:

```bash
npx @modelcontextprotocol/inspector \
  -e MCP_BASE_PATH=/path/to/your/notebook \
